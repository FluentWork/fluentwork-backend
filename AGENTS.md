...
1. 先写测试 —— 一个在当前代码上会**失败**的测试
2. 跑它，**确认它失败**，且失败信息指向真实原因。第一次就通过 = 你没复现问题
3. 再改代码，只改到测试通过
4. 跑 `./scripts/dev-check.sh`
5. **留下测试**，修完不许删

复现方式：优先用本地替身（`httptest` + `coder/websocket` 假服务、可注入失败的 provider、可控时钟），不要因为"要连真供应商"就跳过。本仓已有 `startDuplexMockServer` / `scriptedProvider` / `startVolcDuplexStub` 等脚手架，优先复用。

例外只有三类，且都要在实现说明里写明：新能力（红/绿不适用）、需要真机（改用替身复现）、确实无法自动化（写明手动复现步骤与预期日志）。**"测不了"不是省略的理由。**

实现说明的「测试」一节必须贴出**修复前的实际失败输出**，不要转述。

#### 机械检查

`./scripts/dev-check.sh` 的最后一步是 `scripts/check-defect-discipline.sh`，它检查**本次工作区改动过的**实现说明是否带上了上面那份凭据（`## 测试` / `## 门禁` 小节，或写明『红/绿不适用』）。

范围是工作区而不是全仓历史：这条规则从 `docs/36` 起生效，之前的文档早于规则，追溯会让检查长期红着 —— **一个长期红的检查等于没有检查**。

它只能检查凭据**在不在**，检查不了**诚不诚实**。那是人看的。

### 7. 接缝纪律（新建路径的对应条款）

上一条管的是**修复**：「有人报告了一个问题」时，先写会红的测试。
它管不到**新建**：新功能这条路没有任何同等强度的要求，而 `94_` 审核里的九条
（帧编码两份、turn id 三套、常量四个、错误策略散落）**全部长在新建路径上**。

真源：`docs/99_设计与实现纪律_接缝驱动_2026-09-18.md`。

**动手前先声明接缝。** 每条新增/修改的接缝，写清楚四件事：

```markdown
## 接缝
- 接缝：<HTTP / 包 / goroutine / 表示 / 层>，本次新增还是修改
- 状态所有权：<每条跨接缝的状态，归哪个包或类型>
- 不变量：<这条接缝上什么必须永远成立>
- 证明：<哪个测试会在它被破坏时变红>
```

**最后一行是关键**：写不出测试的"不变量"不是不变量，是愿望。

配套的六条（详见 `docs/99_`）：

| # | 规则 | 一句话 |
|---|---|---|
| R2 | 一份语义一个实现点 | 两处"目前恰好一致"→ 合并，或写一致性测试 |
| R3 | 常量必须有出处 | 用推导（`FrameBytes(100ms)`）而不是复制数字 |
| R4 | 接口只服务今天的消费者 | 指不出生产调用点的方法删掉 |
| R5 | 先写契约测试再写实现 | 跨进程调用**驱动真 handler**，不手写响应体 |
| R6 | 依赖方向要能跑 | 写成 `go list -deps` 之类的可执行判据 |
| R7 | 坑的修法落在结构上 | 问「这个坑的形状是什么」，让形状不可能重现 |

**为什么新建侧需要这条**：结构问题没有触发点——代码重复不会让任何东西变红，
状态分散不会让任何东西变红。没有反馈就没有压力，必然积累。
这七条是把反馈装回去，成本是几行字。

#### 机器执行的部分（每次提交都跑）

R6 不是口号，它已经进了 lint 配置：`.golangci.yml` 的 `depguard` 已经禁止
`internal/voicegateway` import `content/tts` / `reviewgen` / `eval` / `account`。
违反即 `dev-check.sh` 变红。

**检查可以住在任何地方**：单元测试、linter 配置、脚本、SQL。介质不重要，**可执行**才重要
（本仓的三种形态：`sync-shared-schemas.sh` 查协议镜像、`check-duplex-lifetime.py` 查日志不变量、
`check-defect-discipline.sh` 查文档结构）。
...
|---|---|
| 每个改动 | 声明接缝（上面四行） |
| 新模块/新链路动工前 | 写一份接缝契约（照 `docs/88_` 的形状） |
| 每月（搭 `scripts/reflux-report.sh` 的车） | 挑**一条**接缝做人工审计：找并行实现、找两处定义，**产出新检查** |

**每月产出新断言，每天执行已有断言**——混为一谈就白做了。

**审计被打断时**：推迟的项必须写下**归属人 + 复查日期 + 触发条件**，进
`docs/devnotes/README.md` 的未解决清单。案底：`d81e41d` 的防腐层审计点名了语音接缝
「单独评估」，无人跟踪，六天后那条接缝成了 `94_` 的九条发现。

#### 可移植版本

`docs/devnotes/_接缝纪律内核.md` 是一页纸的内核，用于复制到其他项目。
**只搬内核，不搬本仓的案底**——项目特有的那层要在使用中长出来（`docs/100_` §5）。
