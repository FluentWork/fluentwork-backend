...
func TestService_RateLimitSleeps(t *testing.T) {
	...
	if _, _, _, err := store.EndSession(ctx, "s", 4, utts, nil, now, nil); err != nil {
		t.Fatal(err)
	}
...
func TestService_FiftyUtterancesUnderBudget(t *testing.T) {
	...
	if _, _, _, err := store.EndSession(ctx, "big", 50, utts, nil, now, nil); err != nil {
		t.Fatal(err)
	}
...
func endedSession(t *testing.T, id, userID string, utts ...session.Utterance) (*session.MemoryStore, time.Time) {
	...
	if _, _, _, err := store.EndSession(context.Background(), id, 10, utts, nil, now, nil); err != nil {
		t.Fatalf("end: %v", err)
	}
	return store, now
}
