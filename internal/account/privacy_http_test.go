...
func setupPrivacyServer(t *testing.T) (*httpserver.Server, *account.Service, *account.PrivacyService, *corpus.MemoryStore, *account.MemoryStore, *account.TokenResponse) {
	...
	server := httpserver.New(cfg, logger, accountHandler, corpus.NewHandler(corpus.NewService(blocks, logger), accountHandler), nil, nil, nil, nil, drillHandler, nil, nil, nil, accountStore.Ping)
