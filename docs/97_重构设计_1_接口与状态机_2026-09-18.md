...
**状态**：设计，**未动代码** —— 这句话今天仍然成立，但**必须配下面这张对照表读**，否则会去找不存在的类型。

> ### 本文与落地实现对照（2026-09-20 追加）
>
> M1–M6 落地了（`98_` §七），但**落地的不是本文这几个接口**。照本文的名字去 grep 会零命中：
>
> | 本文写的 | 落地情况 | 实际落点 |
> |---|---|---|
> | `DuplexTransport` / `DuplexConn` / `TransportEvent` / `TransportConfig` | **从未实现** | 真实接缝是 `VoiceProvider` / `VoiceProviderSession`（`internal/voicegateway/provider.go`） |
> | `TurnTimeouts`（超时集中声明） | **从未实现** | 超时仍散在 `silence_detector.go` / `collectTurn` 的 wait 参数 / `idleTimeout` |
> | `UtteranceSink` | **从未实现**（没有这一层间接） | 直接 `beginUtterance(...)`（`utterance.go`） |
> | `UtteranceWriter` 的 `Text(delta)` / `Finish(outcome)` | 名字落地、**形状收窄了** | 只剩 `Audio` / `End` / `Interrupt`（`utterance.go`）——文本不进这条流，梯子文本走 `ai.rescue.ladder` |
> | `TurnObserver` / `TurnEnded` | **从未实现** | 特性层仍是直接调用；`noteProviderOutbound` 是那个位置 |
> | `Turn` 状态机 + `transitions` 白名单 | **落地了**，只改了名 | `turnTransitions`（`turn.go`），形状与本文§2.1 一致 |
> | `Abandoned` 态 | **没有这一态** | abort 落到 `TurnClosed`（与 `97_` 的 `Abandoned → Idle` 等价） |
> | 被拒的迁移"记录成证据" | **落地了** | `Turn.rejected map[TurnEvent]int` + `Rejected()`（`turn.go`），对应 `98_` 的 D1 |
> | `onEnterListening` 里的 `firstNonEmpty(ev.TurnID, sessionID)` | 落地了单一回落点，换了位置 | `Turn.ApplySpeechEnd`（客户端给了就用，没给回落 session id） |
> | §三 的"可注入时钟" | 落地的可注入量是**节奏**不是时钟 | `streamWriter.pace`（`time.Sleep(w.pace)` 仍是真实 sleep） |
>
> **这份差异本身就是证据**：`98_` §五 记着为什么"给 AI 发 `ai.tts.start`"当时被否掉，
> 而最终恰恰是换了一条路（删掉客户端回退路径）才让它成立。读者不该把本文当作代码地图。
