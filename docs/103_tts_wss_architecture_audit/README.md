...
> **⚠️ 时点声明（2026-09-25 补）**：本系列是 **2026-09-21 的快照**，结论与数字都停在那一天。
> 其中最容易被误引的是测试数量：本系列写的「208 个 / 208+ 个」是当时的计数，
> 现在全仓是 **754 个 `func Test`**（当时 746 的记法见 `104_架构分析/05`）。
> 另有 `voicegateway` 的扁平包规模、迁移机制等条目，本系列**不在其范围内**，
> 现状以 [`../104_架构分析/05_问题清单与建议.md`](../104_架构分析/05_问题清单与建议.md) 为准。
> 本系列的价值是「那条链路做到设计了没有」，答案是「做到了」——这一条没有变。
